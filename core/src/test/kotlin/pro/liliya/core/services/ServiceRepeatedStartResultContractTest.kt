...
class ServiceRepeatedStartResultContractTest {
    @Test
    fun repeated_start_reports_empty_applied_set() {
        ...
        val context = LogContextPropagation.root(
            module = "CORE",
            component = "Services",
            operation = "repeated-start-result",
            generator = CorrelationIdGenerator { "service-repeated-start-result" }
        )

        assertIs<ServiceLifecycleResult.Applied>(manager.startAll(context))
        val repeated = manager.startAll(context)
