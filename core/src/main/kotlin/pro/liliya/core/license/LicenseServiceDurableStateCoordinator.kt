...
sealed interface LicenseServiceDurableInitializationResult {
    class Restored internal constructor(
        val snapshot: LicenseServiceDurableStateSnapshot
    ) : LicenseServiceDurableInitializationResult {
        override fun toString(): String =
            "LicenseServiceDurableInitializationResult.Restored(snapshot=$snapshot)"
    }

    class AlreadyInitialized internal constructor(
        val snapshot: LicenseServiceDurableStateSnapshot
    ) : LicenseServiceDurableInitializationResult {
        override fun toString(): String =
            "LicenseServiceDurableInitializationResult.AlreadyInitialized(snapshot=$snapshot)"
    }

    data object Missing : LicenseServiceDurableInitializationResult
...
class LicenseServiceDurableStateCoordinator(
    ...
    /**
     * Restores previously committed authenticated security state without requiring a new
     * Licensing Service response.
     *
     * This is the explicit process-start boundary required for offline policy evaluation.
     * Missing durable state is not an error. Corrupt, incompatible or unauthenticated state
     * fails closed and never publishes partial state.
     */
    fun initialize(): LicenseServiceDurableInitializationResult = synchronized(lock) {
        published?.let {
            return@synchronized LicenseServiceDurableInitializationResult.AlreadyInitialized(it)
        }
        if (initializationUncertain) {
            return@synchronized LicenseServiceDurableInitializationResult.Rejected(
                LicenseServiceDurableStateFailure.INITIALIZATION_UNCERTAIN
            )
        }

        when (val loaded = loadCurrent()) {
            CurrentLoadResult.Missing -> LicenseServiceDurableInitializationResult.Missing

            is CurrentLoadResult.Rejected ->
                LicenseServiceDurableInitializationResult.Rejected(loaded.reason)

            is CurrentLoadResult.Present -> {
                published = loaded.snapshot
                observeRestored(loaded.snapshot)
                LicenseServiceDurableInitializationResult.Restored(loaded.snapshot)
            }
        }
    }
    ...
    private fun observeRestored(snapshot: LicenseServiceDurableStateSnapshot) {
        foundation.observability.record(
            severity = DiagnosticSeverity.INFO,
            code = "LICENSE_SERVICE_DURABLE_STATE_RESTORED",
            message = "license service durable security state restored",
            context = foundation.rootContext(
                operation = "initializeDurableLicenseServiceState",
