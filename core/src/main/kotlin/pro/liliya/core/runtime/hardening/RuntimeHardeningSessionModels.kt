...
enum class RuntimeModelSessionLifecycle { PREPARED, ACTIVE, QUIESCING, FAILED, RETIRED }

enum class RuntimeHardeningFailure {
    ACTIVATION_REJECTED, ACTIVATION_FAILED, SESSION_STALE, SESSION_UNAVAILABLE, SESSION_FAILED,
    RESOURCE_LIMIT_REJECTED, OPERATION_REJECTED, OPERATION_FAILED, OPERATION_CANCELLED,
    OPERATION_TIMEOUT, RETIREMENT_FAILED, RECOVERY_REJECTED, PROVIDER_FAILED
}
...
enum class RuntimeSessionRegistrationFailure { LIVE_SESSION_EXISTS, GENERATION_OVERFLOW }
...
class RuntimeModelSessionRegistry internal constructor(initialGeneration: Long = 0L) {
    ...
    fun register(id: RuntimeModelSessionId, model: ProtectedModelReference): RuntimeSessionRegistrationResult = synchronized(lock) {
        check(current?.publicationInProgress != true) { "runtime session registration is not allowed from inside publication" }
        if (current != null) return@synchronized RuntimeSessionRegistrationResult.Rejected(RuntimeSessionRegistrationFailure.LIVE_SESSION_EXISTS)
        val nextValue = nextGeneration.incrementAndGet()
        if (nextValue <= 0L) return@synchronized RuntimeSessionRegistrationResult.Rejected(RuntimeSessionRegistrationFailure.GENERATION_OVERFLOW)
        val entry = Entry(RuntimeModelSessionReference(id, RuntimeModelSessionGeneration(nextValue), model))
        current = entry
    ...
    fun currentFailure(): RuntimeHardeningFailure? = synchronized(lock) { current?.failure }
    ...
    internal fun claimOperationSupervisor(): Boolean = synchronized(lock) {
        if (operationSupervisorClaimed) false else { operationSupervisorClaimed = true; true }
    }

    internal fun <T> withCurrentActiveSession(block: (RuntimeModelSessionReference) -> T): RuntimeActiveSessionGuardResult<T> = synchronized(lock) {
        val entry = current
        if (entry == null || entry.lifecycle != RuntimeModelSessionLifecycle.ACTIVE || entry.publicationInProgress) RuntimeActiveSessionGuardResult.Unavailable
        else RuntimeActiveSessionGuardResult.Available(block(entry.reference))
    }

    internal fun beginQuiescingIfCurrent(reference: RuntimeModelSessionReference): RuntimeSessionQuiescingTransitionResult = synchronized(lock) {
        val entry = current
        if (entry == null || entry.reference != reference) return@synchronized RuntimeSessionQuiescingTransitionResult.Stale
        check(!entry.publicationInProgress) { "runtime session quiescing is not allowed from inside publication" }
        when (entry.lifecycle) {
            RuntimeModelSessionLifecycle.ACTIVE -> { entry.lifecycle = RuntimeModelSessionLifecycle.QUIESCING; RuntimeSessionQuiescingTransitionResult.Quiescing }
            RuntimeModelSessionLifecycle.QUIESCING -> RuntimeSessionQuiescingTransitionResult.AlreadyQuiescing
            else -> RuntimeSessionQuiescingTransitionResult.Stale
        }
    }

    internal fun failIfCurrent(reference: RuntimeModelSessionReference, reason: RuntimeHardeningFailure): RuntimeSessionFailureTransitionResult = synchronized(lock) {
        ...
        val entry = current
        if (entry == null || entry.reference != reference) return@synchronized RuntimeSessionFailureTransitionResult.Stale
        check(!entry.publicationInProgress) { "runtime session failure transition is not allowed from inside publication" }
        when (entry.lifecycle) {
            RuntimeModelSessionLifecycle.ACTIVE, RuntimeModelSessionLifecycle.QUIESCING -> {
                entry.lifecycle = RuntimeModelSessionLifecycle.FAILED; entry.failure = reason
                RuntimeSessionFailureTransitionResult.Failed(reason)
            }
            RuntimeModelSessionLifecycle.FAILED -> RuntimeSessionFailureTransitionResult.AlreadyFailed(checkNotNull(entry.failure))
    ...
    internal fun isCurrentQuiescing(reference: RuntimeModelSessionReference): Boolean = synchronized(lock) {
        current?.let { it.reference == reference && it.lifecycle == RuntimeModelSessionLifecycle.QUIESCING && !it.publicationInProgress } == true
    }

    internal fun retireQuiescingIfCurrent(reference: RuntimeModelSessionReference, retire: () -> Unit): RuntimeSessionRetirementTransitionResult = synchronized(lock) {
        val entry = current
        if (entry == null || entry.reference != reference || entry.lifecycle != RuntimeModelSessionLifecycle.QUIESCING) return@synchronized RuntimeSessionRetirementTransitionResult.Stale
        check(!entry.publicationInProgress) { "runtime session retirement is not allowed from inside publication" }
        ...
        } finally { entry.publicationInProgress = false }
    }

    internal fun retireFailedIfCurrent(reference: RuntimeModelSessionReference): Boolean = synchronized(lock) {
        val entry = current
        if (entry == null || entry.reference != reference || entry.lifecycle != RuntimeModelSessionLifecycle.FAILED) return@synchronized false
        check(!entry.publicationInProgress) { "failed runtime session retirement is not allowed from inside publication" }
        if (entry.failure == RuntimeHardeningFailure.RETIREMENT_FAILED) return@synchronized false
        entry.lifecycle = RuntimeModelSessionLifecycle.RETIRED
        current = null
        true
    }
    ...
    internal fun recoverRetirementFailureIfCurrent(
        ...
        val entry = current
        if (entry == null || entry.reference != reference || entry.lifecycle != RuntimeModelSessionLifecycle.FAILED || entry.failure != RuntimeHardeningFailure.RETIREMENT_FAILED) {
            return@synchronized RuntimeSessionRetirementTransitionResult.Stale
        }
        check(!entry.publicationInProgress) { "retirement recovery is not allowed from inside publication" }
        entry.publicationInProgress = true
        try {
            recoverCleanup()
            entry.lifecycle = RuntimeModelSessionLifecycle.RETIRED
    ...
    internal fun publishOperationIfCurrent(reference: RuntimeModelSessionReference, publish: () -> Unit): RuntimeOperationPublicationResult = synchronized(lock) {
        val entry = current
        if (entry == null || entry.reference != reference || entry.lifecycle != RuntimeModelSessionLifecycle.ACTIVE) return@synchronized RuntimeOperationPublicationResult.Stale
        check(!entry.publicationInProgress) { "nested runtime session publication is not allowed" }
        entry.publicationInProgress = true
        try {
            publish()
            if (current !== entry || entry.lifecycle != RuntimeModelSessionLifecycle.ACTIVE) RuntimeOperationPublicationResult.Stale else RuntimeOperationPublicationResult.Published
        } catch (throwable: Throwable) { RuntimeOperationPublicationResult.Failed(throwable) }
        finally { entry.publicationInProgress = false }
    }

    private fun ownership(entry: Entry): RuntimeModelSessionOwnership = object : RuntimeModelSessionOwnership {
        override val reference = entry.reference
        ...
        override fun publishIfCurrent(publish: () -> Unit): RuntimeSessionPublicationResult = synchronized(lock) {
            if (current !== entry || entry.lifecycle != RuntimeModelSessionLifecycle.PREPARED) return@synchronized RuntimeSessionPublicationResult.Stale
            check(!entry.publicationInProgress) { "nested runtime session publication is not allowed" }
            entry.publicationInProgress = true
            try {
                publish()
                if (current !== entry) return@synchronized RuntimeSessionPublicationResult.Stale
                entry.lifecycle = RuntimeModelSessionLifecycle.ACTIVE
                RuntimeSessionPublicationResult.Published
            } catch (throwable: Throwable) {
                if (current === entry) { entry.lifecycle = RuntimeModelSessionLifecycle.FAILED; entry.failure = RuntimeHardeningFailure.ACTIVATION_FAILED; current = null }
        ...
        override fun retire(): Boolean = synchronized(lock) {
            ...
            if (operationSupervisorClaimed && entry.lifecycle != RuntimeModelSessionLifecycle.PREPARED) return@synchronized false
            entry.lifecycle = RuntimeModelSessionLifecycle.RETIRED; current = null; true
        }
    }
}
