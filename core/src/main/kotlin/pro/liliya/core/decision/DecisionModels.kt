...
@JvmInline
value class DecisionSourceId(val value: String) {
    init { require(value.isNotBlank()) { "decision source id must not be blank" } }
    override fun toString(): String = value
}

@JvmInline
value class DecisionSourceReference(val value: String) {
    init { require(value.isNotBlank()) { "decision source reference must not be blank" } }
    override fun toString(): String = value
}
...
sealed interface DecisionInputReference {
    ...
    data class Declared(
        val sourceId: DecisionSourceId,
        val sourceReference: DecisionSourceReference? = null
    ) : DecisionInputReference
}
...
class DecisionRecord(
    ...
    init {
        require(this.inputs.isNotEmpty()) { "decision must contain at least one input reference" }
        ...
        require(this.options.any { it.id == selectedOptionId }) {
            "selected decision option must exist in options"
        }
    ...
    override fun equals(other: Any?): Boolean =
        other is DecisionRecord &&
            ...
            selectedOptionId == other.selectedOptionId && rationale == other.rationale && createdAt == other.createdAt
...
data class DecisionRecordSnapshot(
