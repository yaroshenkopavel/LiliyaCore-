...
internal interface DecisionRecordRegistration {
    val decision: DecisionRecord
    val generation: DecisionGeneration
    fun remove(context: LogContext): Boolean
}

internal sealed interface DecisionRecordRegistrationResult {
    data class Registered(val registration: DecisionRecordRegistration) : DecisionRecordRegistrationResult
    data class Rejected(val reason: String) : DecisionRecordRegistrationResult
}

internal class DecisionRecordStore(
    ...
    fun register(
        ...
        context: LogContext
    ): DecisionRecordRegistrationResult {
        val entry = Entry(
        ...
        if (previous != null) {
            ...
            return DecisionRecordRegistrationResult.Rejected(reason)
        }
        ...
        return DecisionRecordRegistrationResult.Registered(
            object : DecisionRecordRegistration {
    ...
    fun inspect(id: DecisionId): DecisionRecordSnapshot? = decisions[id]?.let { entry ->
        DecisionRecordSnapshot(entry.decision, entry.generation)
    }
    ...
    fun snapshotEntries(): List<DecisionRecordSnapshot> = decisions.values
        .map { DecisionRecordSnapshot(it.decision, it.generation) }
        .sortedWith(compareBy<DecisionRecordSnapshot> { it.decision.createdAt }.thenBy { it.decision.id.value })
    ...
    private fun metadata(
        ...
        put("decisionSelectedOptionId", decision.selectedOptionId.value)
